...
// fallback if .env.local didn't load
if (!process.env.NEXT_PUBLIC_SUPABASE_URL) {
  process.env.NEXT_PUBLIC_SUPABASE_URL =
    "https://eaxzcobkrcgrceojrqed.supabase.co";
}
if (!process.env.SUPABASE_SERVICE_ROLE_KEY) {
  process.env.SUPABASE_SERVICE_ROLE_KEY = "your_key_here"; // use your real key
}
...
async function seedJobPostings() {
  ...
  const jobPostings = employers.map((emp) => ({
    ...
    salary_range: `$${faker.number.int({
      min: 50000,
      max: 150000,
    })} - $${faker.number.int({ min: 60000, max: 160000 })}`,
    status: faker.helpers.arrayElement(["open", "closed", "pending"]),
    posted_at: new Date().toISOString(),
    deadline: faker.date.future().toISOString().split("T")[0],
    expected_skills: faker.helpers
      .arrayElements(
        ["JavaScript", "React", "Node.js", "Python", "SQL", "TensorFlow"],
        3
      )
      .join(", "),
    company_ID: emp.id,
